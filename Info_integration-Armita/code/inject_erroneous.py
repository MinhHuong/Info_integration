...
def inject(input_path,output_path,source,target): #source and target are 2 lists
	'''
	input : input_path: path to refalign
			output_path: path to save refalign with injected errors
			source: a list containing random subjects from source ontology (000)
			target: a list containing random subjects from target ontology (001/ 002)
	return : None		
...
def correct_sameAs(path_refalign):
	'''
	input : path to refalign ex: '../data/001/refalign.rdf'

	This method parses the refalign file with minidom parser for XML

	return : sameAs : A dictionary hat keeps correct SameAs Links between
